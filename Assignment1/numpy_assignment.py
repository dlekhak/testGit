...
def xmatrix(N):
    myArr = np.zeros((N,N), dtype=int) 
    np.fill_diagonal(myArr,1)
    np.eye(N)[::-1]
    return myArr
...
def min_row_col(m):
    min_row = np.argmin(np.sum(m, axis=1))
    min_col = np.argmin(np.sum(np.transpose(m), axis=1))
...
def fix_gauge_bias(car_data, speed_bias, rpm_bias):
    car_data[::,0:1] = car_data[::,0:1] - speed_bias
...
def was_gear_switched(car_data):
    max_val = np.max(car_data[::,1:]/car_data[::,0:1]) 
    min_val = np.min(car_data[::,1:]/car_data[::,0:1])
...
def count_gears_used(car_data):
    return len(np.unique(np.array([(car_data[i][1]//car_data[i][0]) for i in range(len(car_data))]))) -1
